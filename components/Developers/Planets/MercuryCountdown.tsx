// src/components/Developers/Planets/MercuryCountdown.tsx
'use client';
import React, { useState, useEffect } from 'react';
import { planetsData } from '@/data/starData';
const MercuryCountdown: React.FC = () => {
    const [timeLeft, setTimeLeft] = useState<string>('');
    const planetData = planetsData.find(p => p.id === 'mercury');
    useEffect(() => {
        if (!planetData) {
            setTimeLeft('ไม่พบข้อมูลดาวพุธ');
            return;
        }
...
export default MercuryCountdown;