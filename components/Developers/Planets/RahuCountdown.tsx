// src/components/Developers/Planets/RahuCountdown.tsx
'use client';
import React, { useState, useEffect } from 'react';
import { planetsData } from '@/data/starData';
const RahuCountdown: React.FC = () => {
    const [timeLeft, setTimeLeft] = useState<string>('');
    const planetData = planetsData.find(p => p.id === 'rahu');
    useEffect(() => {
        if (!planetData) {
            setTimeLeft('ไม่พบข้อมูลดาวราหู');
            return;
        }
