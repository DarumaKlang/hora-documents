// src/components/Developers/Planets/VenusCountdown.tsx
'use client';
import React, { useState, useEffect } from 'react';
import { planetsData } from '@/data/starData';
const VenusCountdown: React.FC = () => {
    const [timeLeft, setTimeLeft] = useState<string>('');
    const planetData = planetsData.find(p => p.id === 'venus');
    useEffect(() => {
        if (!planetData) {
            setTimeLeft('ไม่พบข้อมูลดาวศุกร์');
            return;
        }
