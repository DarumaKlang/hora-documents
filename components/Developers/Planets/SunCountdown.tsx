// src/components/Developers/Planets/SunCountdown.tsx
'use client';
import React, { useState, useEffect } from 'react';
import { planetsData } from '@/data/starData';
const SunCountdown: React.FC = () => {
    const [timeLeft, setTimeLeft] = useState<string>('');
    const planetData = planetsData.find(p => p.id === 'sun');
    useEffect(() => {
        if (!planetData) {
            setTimeLeft('ไม่พบข้อมูลดวงอาทิตย์');
            return;
        }
        ...
        const intervalId = setInterval(() => {
            ...
            if (distance > 0) {
                ...
                setTimeLeft(`ถึงเวลาที่ดวง ${planetData.name} ย้ายเข้าสู่ ${nextTransit.sign} แล้ว!`);
            }
        }, 1000);
