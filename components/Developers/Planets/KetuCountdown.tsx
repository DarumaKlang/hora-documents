// src/components/Developers/Planets/KetuCountdown.tsx
'use client';
import React, { useState, useEffect } from 'react';
import { planetsData } from '@/data/starData';
const KetuCountdown: React.FC = () => {
    const [timeLeft, setTimeLeft] = useState<string>('');
    const planetData = planetsData.find(p => p.id === 'ketu');
    useEffect(() => {
        if (!planetData) {
            setTimeLeft('ไม่พบข้อมูลดาวเกตุ');
            return;
        }
