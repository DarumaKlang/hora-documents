...
const SevenNineCard: React.FC = () => {
    return (
        <div className="flex flex-col items-center font-sans bg-white/5 backdrop-blur-md p-2 md:p-6 rounded-lg shadow-xl border border-secondary-gold/30">
            <h1 className="text-xl sm:text-2xl md:text-3xl text-teal-800 font-bold mb-4 md:mb-8 drop-shadow-lg text-center">
                ตารางมหาสัตตเลข ๗ ตัว ๙ ฐาน
            </h1>
            <div className="overflow-x-auto w-full max-w-4xl">
                <div className="grid grid-cols-7 gap-1 w-full min-w-[320px] mx-auto">
                    {initialTableData.map((row, rowIndex) => (
                        row.cells.map((cell, cellIndex) => (
                            <div
                                key={`${rowIndex}-${cellIndex}`}
                                className={`p-1 text-center border border-gray-300 rounded-md text-[8px] sm:text-[10px] md:text-xs lg:text-sm ${valueToColorMap[cell.value] || 'bg-gray-500'}`}
                            >
                                <div className="text-gray-100">{cell.title}</div>
                                <div className="font-bold text-xs md:text-sm mt-1">{toThaiNumeral(cell.value)}</div>
                            </div>
                        ))
                    ))}
                </div>
